...
int main ()
{
	...
	for (std::map<char,int>::iterator it=five.begin(); it!=five.end(); ++it)
		std::cout << it->first << " => " << it->second << '\n';
	...
	const std::map<char, int> test = second;

	std::cout << "\nThe const reverse contents of const second(test) are: ";
	for (std::map<char, int>::const_reverse_iterator it=test.rbegin(); it!=test.rend(); ++it)
		std::cout << it->first << " => " << it->second << std::endl;
