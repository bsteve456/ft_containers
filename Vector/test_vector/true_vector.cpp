...
int main()
{
	...
	const std::vector<int> test = myvector;
	std::cout << "The const reverse contents of const myvector(test) are:";
	for (std::vector<int>::const_reverse_iterator it=test.rbegin(); it!=test.rend(); ++it)
		std::cout << ' ' << *it;
	std::cout << '\n';
	...
	for (std::vector<int>::const_iterator it = ba.begin(); it!=ba.end(); ++it)
		std::cout << ' ' << *it;
	std::cout << '\n';
